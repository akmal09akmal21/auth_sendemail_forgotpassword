...
function sendOTP(email, otp) {
  ...
  transporter.sendMail(mailOptions, (error, info) => {
    if (error) {
      ...
      console.log("OTP Email succesfully", info.response);
    }
  });
}
// route request otp
router.post("/reqotp", (req, res) => {
  ...
  res.status(200).send({ message: "otp sent success", success: true });
});
...
router.post("/verifyotp", (req, res) => {
  ...
  if (otpCache[email] === otp.trim()) {
    // remove otp from cache after successfully verification
    delete otpCache[email];
    return res
      .status(200)
      .send({ success: true, message: "OTP verified success" });
  } else {
